...
def get_structure(formula, prototype, base_dir="./", c=None, **filters):
    ...
    res = list(db.select(formula=formula, prototype=prototype))
    if len(res) > 1:
        parprint(("Multiple candidates with "
                  "formula {} and prototype {}!").format(formula, prototype))
    for mol in res:
        symbol = mol.formula
        pos = mol.positions
        cell = mol.cell
